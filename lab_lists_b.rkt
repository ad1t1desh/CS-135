...
;; 2 PTS
(define (yoda words)
  (list(car(cdr(cdr words))) (car words) (car(cdr words))))
...
(define (quad-roots a b c)
  (reverse(list(/(+ (* b -1) (sqrt(-(expt b 2) (* 4 a c)))) (* 2 a)) (/(- (* b -1) (sqrt(-(expt b 2) (* 4 a c)))) (* 2 a)))))
;; 
...
;; 2 PTS
(define (rotate c)
 (list(list (car c)(car(cdr c))) (list(car(cdr c)) (* -1 (car c))) (list (* -1 (car c)) (* -1 (car(cdr c)))) (list (* -1 (car(cdr c))) (car c))))
...
(define (combine-products b1 b2)
  (append (cadr(cddddr b1)) (cadr(cddddr b2))))
;;
