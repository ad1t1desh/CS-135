...
#| Implement "piecewise" to accept an integer x
 |   and return 3*(x^2) + 1 if x is positive,
 |   and x + 2 otherwise.
 | Examples:
 |   (piecewise -5) -> -3
 |   (piecewise -2) -> 0
...
;; 1 PTS
(define (sign x)
  (if (> x 0) (write 'positive)(if (= x 0) (write 'zero) (write 'negative))))
...
#| Implement "sum-power" to accept positive integers n and p
 ...
 |   (sum-power 15 2) -> 1240
 |   (sum-power 8 5)  -> 61776
 |   (sum-power 7 10) -> 353815700
 |#

;; Type signature: (sum-power positive-int positive-int) -> int
;; 3 PTS
(define (sum-power n p)
  (if (<= n 0)
      0
      (+ (expt n p) (sum-power (- n 1) p))))
...
(define (gen-fib a b n)
  (if (= n 0)
      ...
      (if (= n 1)
          b
          (+ (gen-fib a b (- n 2))(gen-fib a b (- n 1))))))
...
(define (filter P L)
  (if (null? L)
      '()
      (if (equal? (P(car L)) #t)
          (append (list(car L)) (filter P (cdr L)))
          (filter P (cdr L)))))
