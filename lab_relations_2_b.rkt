...
#|-------------------------------------------------------------------------------|
 |                  Lab 7: Relations Part II (12 PTS + 3 EC)                      |
 |-------------------------------------------------------------------------------|#
...
#| HELPER FUNCTION
 |"compose" accepts two relations S and R,
 |   and returns S ∘ R, or S composed with R.
...
(define (transitive-closure R)
  (help2 (cardinality R) R))
   
 
(define (help counter R)
  (if (equal? counter 0) '()
      (if(equal? counter 1) R
        (compose R (help (- counter 1) R)))))

(define (help2 p R)
   (if (equal? p 0)
       '()
       (union (help p R) (help2 (- p 1) R))))
...
;; 3 PTS
(define (transitive? R)
  (if (set-equal? R (transitive-closure R))
      #t
      #f))
...
(define (equivalence? n R)
  (if (and (symmetric? R) (reflexive? n R) (transitive? R))
      #t
      #f))
...
#| OPTIONAL:
 | Implement "involutive?" to accept a relation R and a positive int n
